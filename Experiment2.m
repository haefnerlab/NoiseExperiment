...
function [outputs] = Experiment2(Images, map, ratios)
...
experiment_type_contrast = [image_contrast_initial, image_contrast_initial, image_contrast_initial, image_contrast_initial];
...
for i =1:trials,
    ...
    if strcmp(experiment_type, 'both')==1,
        first_image = squeeze(Images(f, map(experiment_type_contrast(stair_case), f), :, :)); 
        %double(imread(strcat('ImageLibrary/', first_noise, '/Contrast',num2str(image_contrast), '/Image',num2str(randi(1000)), '.png')));
        second_image =squeeze(Images(w, map(experiment_type_contrast(stair_case), w), :, :)); 
    ...
    elseif strcmp(experiment_type, 'white-2AFC')==1
        ...
        second_noise = 'white';
         if first_type ==2,
            first_image =squeeze(Images(w, map((experiment_type_contrast(stair_case)+ uint32(normrnd(0,2))), w), :, :));
           second_image =squeeze(Images(w, map(experiment_type_contrast(stair_case), w), :, :));
        else
           first_image = squeeze(Images(f, map(experiment_type_contrast(stair_case), f), :, :)); 
            second_image =squeeze(Images(f, map((experiment_type_contrast(stair_case)+ uint32(normrnd(0,2))), f), :, :)); 
         end
	...
	Trial( stair_case,counter(stair_case)).Contrast = experiment_type_contrast(stair_case)
	Trial( stair_case,counter(stair_case)).First_Likelihood = ratios(f, map(experiment_type_contrast(stair_case), f));	
	Trial( stair_case,counter(stair_case)).Second_Likelihood = ratios(w, map(experiment_type_contrast(stair_case), w));	
	...
	if(Trial(stair_case,counter(stair_case)).Orientation_Correct)
		...
		if(counter(stair_case)>1 && Trial(stair_case, counter(stair_case)-1).Orientation_Correct)
			experiment_type_contrast(stair_case) = max(experiment_type_contrast(stair_case)-25+ uint32(normrnd(0, 2)) , 1);
		...
		PsychPortAudio('Start', handle, 1, 0, 1);
			...
			PsychPortAudio('Stop', handle);
		if(i>1)
			experiment_type_contrast(stair_case) = min(experiment_type_contrast(stair_case) +25 + uint32(normrnd(0,2)), 11000);
		end
	end
